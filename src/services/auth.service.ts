import { LoginResponse, SignupResponse } from "../interfaces/response/auth.response";
...
export const checkUser = async (credential: string): Promise<IUserSchema | null> => {
...
export const loginUser = async ({ credential, password }: LoginBody): Promise<LoginResponse | undefined> => {
    ...
    const token = await signJwt(user?.email as string, user?._id?.toString() as string);
...
export const registerUser = async ({
    ...
    if (!email || !password || !username) {
        throw new BadRequestError("Provide username,firstname, lastname, email and pasword");
    }
    ...
    const token = await signJwt(userAuthInstance.email, userAuthInstance._id.toString());
